...
\newcommand\LMUIFI{Institut f\"ur Informatik, Ludwig-Maximilians-Universit\"at M\"unchen}
...
\author{\hbox{} \\
Jasmin Blanchette \\
{\normalsize \LMUIFI} \\[4\smallskipamount]
...
veriT, Z3, Zipperposition, and proof methods to be run in parallel, either
locally or remotely---depending on the number of processor cores available and
on which provers are actually installed. The subset includes all proof methods
called by the \textbf{try0} command. (Proof methods can appear in Isabelle
proofs that reconstruct proofs originally found by ATPs or SMT solvers.)
