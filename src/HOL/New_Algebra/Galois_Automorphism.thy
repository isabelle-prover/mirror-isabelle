...
lemma field_auto_zero:
  ...
  shows "\<sigma> 0 = 0"
  using Ksub s by (metis Subfield.zero_closed add_cancel_right_right field_auto_mem_iff)
...
lemma field_auto_closed:
  ...
  shows "\<sigma> x \<in> K"
  using assms bij_betwE by (fastforce simp: field_auto_def)
...
lemma field_auto_uminus:
  assumes "Subfield K" "\<sigma> \<in> field_auto K F" and x: "x \<in> K"
  shows "\<sigma> (-x) = -\<sigma> x"
  using assms Subfield.uminus_closed add_eq_0_iff2 field_auto_add field_auto_zero x
  by fastforce
...
lemma field_auto_sum:
  ...
  using assms field_auto_zero
  by (induction J rule: infinite_finite_induct) (auto simp add: Subfield.sum_closed field_auto_add)
...
proof -
  ...
  proof 
    show "\<sigma> 0 = 0"
      using Ksub field_auto_zero s by blast
    show "\<sigma> 1 = 1"
      using Ksub field_auto_one s by blast
  qed (use s in \<open>auto simp: field_auto_def\<close>)
qed
...
proof -
  ...
  proof (intro conjI ballI)
    ...
    show "\<And>x. x \<in> F \<Longrightarrow> ?c x = x"
      using FK sfix tfix by (metis compose_eq in_mono)
  qed
qed
...
proof -
  ...
  define t where "t \<equiv> restrict (inv_into K \<sigma>) K"
  ...
  proof (intro conjI strip)
    show "bij_betw (restrict (inv_into K \<sigma>) K) K K"
      by (simp add: bij bij_betw_inv_into)
    show "restrict (inv_into K \<sigma>) K \<in> K \<rightarrow>\<^sub>E K"
      by (simp add: bij bij_betw_imp_funcset bij_betw_inv_into)
    show "\<And>x y. x \<in> K \<Longrightarrow> y \<in> K \<Longrightarrow>
           restrict (inv_into K \<sigma>) K (x + y) = restrict (inv_into K \<sigma>) K x + restrict (inv_into K \<sigma>) K y"
      using KS.add_closed addh st t_def tin ts by metis
    show "\<And>x y. x \<in> K \<Longrightarrow> y \<in> K \<Longrightarrow>
           restrict (inv_into K \<sigma>) K (x * y) = restrict (inv_into K \<sigma>) K x * restrict (inv_into K \<sigma>) K y"
      using KS.mult_closed mulh st t_def tin ts by metis
    show "restrict (inv_into K \<sigma>) K 1 = 1"
      using one t_def ts by fastforce
    show "\<And>x. x \<in> F \<Longrightarrow> restrict (inv_into K \<sigma>) K x = x"
      using FK fixed t_def ts by (metis subsetD)
  qed
qed
...
proof -
  have "bij_betw a K K" using assms by (rule T.Units_bij_betwI)
  then show ?thesis
    by (simp add: bij_betw_imp_funcset bij_betw_inv_into)
qed
...
proof (intro T.symmetric.inverse_equality field_auto_restrict_inv_in_Sym assms)
  show "compose K a (restrict (inv_into K a) K) = identity K"
    by (simp add: assms bij_betw_imp_surj_on compose_id_inv_into)
...
definition fixed_field ::
  ...
  "fixed_field K H \<equiv> {x \<in> K. \<forall>\<sigma> \<in> H. \<sigma> x = x}"

lemma fixed_field_subset: "fixed_field K H \<subseteq> K"
  by (auto simp: fixed_field_def)

lemma fixed_field_memI:
  "\<lbrakk> x \<in> K; \<And>\<sigma>. \<sigma> \<in> H \<Longrightarrow> \<sigma> x = x \<rbrakk> \<Longrightarrow> x \<in> fixed_field K H"
...
lemma field_auto_fixed_set_subfield:
  assumes Ksub: "Subfield K" and s: "\<sigma> \<in> field_auto K F"
  shows "Subfield {x \<in> K. \<sigma> x = x}" (is "Subfield ?KK")
proof -
  interpret KS: Subfield K by (rule Ksub)
  have add: "\<And>x y. x \<in> K \<Longrightarrow> y \<in> K \<Longrightarrow> \<sigma> (x + y) = \<sigma> x + \<sigma> y"
  ...
  show ?thesis
  proof
    show "0 \<in> ?KK" using KS.zero_closed zero by simp
    show "1 \<in> ?KK" using KS.one_closed one by simp
  next
    fix x assume x: "x \<in> ?KK"
    show "-x \<in> ?KK" "inverse x \<in> ?KK"
      using x Ksub field_auto_uminus field_auto_inverse_on s by fastforce+
    fix y assume y: "y \<in> ?KK"
    show "x + y \<in> ?KK" "x * y \<in> ?KK"
      using x y by (auto simp: add mul)
  qed
qed
...
proof -
  ...
  proof
    show "0 \<in> fixed_field K H"
      using H KS.zero_closed Ksub by (meson field_auto_zero fixed_field_memI subsetD)
    show "1 \<in> fixed_field K H"
      using H KS.one_closed by (metis field_auto_mem_iff fixed_field_memI subsetD)
  next
    ...
    proof (rule fixed_field_memI)
      ...
      then have add: "\<sigma> (x + y) = \<sigma> x + \<sigma> y"
        using H field_auto_add xK yK by blast
      show "\<sigma> (x + y) = x + y"
        using add s x y by (metis fixed_field_memD)
    qed
    show "x * y \<in> fixed_field K H"
    proof (rule fixed_field_memI)
      show "x * y \<in> K" by (rule KS.mult_closed[OF xK yK])
      fix \<sigma> assume s: "\<sigma> \<in> H"
      then have mul: "\<sigma> (x * y) = \<sigma> x * \<sigma> y"
        using H field_auto_mult xK yK by blast
      then show "\<sigma> (x * y) = x * y"
        using s x y by (metis fixed_field_memD)
    qed
  next
    ...
    proof (rule fixed_field_memI)
      ...
      fix \<sigma> assume s: "\<sigma> \<in> H"
      show "\<sigma> (-x) = -x"
        using H Ksub field_auto_uminus fixed_field_memD s x by fastforce
    qed
    ...
    proof (rule fixed_field_memI)
      ...
      show "\<sigma> (inverse x) = inverse x"
        using H Ksub field_auto_inverse_on fixed_field_memD s x by fastforce
    qed
  qed
qed
...
proof -
  ...
  with gen \<open>x \<in> K\<close> show ?thesis by auto
qed
