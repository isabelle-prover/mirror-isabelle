...
text \<open>For a \<^emph>\<open>simple normal\<close> extension
...
proof -
  have "minpoly F a \<noteq> 0" by (rule Subfield.minpoly_nonzero[OF sfF alg])
  moreover have "degree (minpoly F a) > 0"
    using Subfield.is_minpoly_minpoly Subfield.minpoly_degree_pos alg sfF by blast
  ultimately show ?thesis
    unfolding irreducible_over_def
    using alg sfF by (metis Subfield.is_minpoly_minpoly Subfield.minpoly_no_proper_factor)
qed
...
lemma field_auto_determined_by_gen:
  ...
  shows "\<sigma> = \<tau>"
proof -
  define E where "E \<equiv> eval_img F a"
  ...
  have homs: "field_hom_on E \<sigma>"
    using E_def field_auto_imp_field_hom_on s sfE by blast
  have homt: "field_hom_on E \<tau>"
    using E_def field_auto_imp_field_hom_on sfE t by blast
  ...
  proof -
    obtain p where p: "p \<in> poly_over F" and xp: "x = poly p a" and pE: "p \<in> poly_over E"
      using xE poly_over_mono[OF FE] by (auto simp: E_def eval_img_def)
    \<comment> \<open>@{term \<sigma>} commutes with evaluation, and fixes the (\<open>F\<close>-)coefficients.\<close>
    have "\<sigma> x = poly (map_poly \<sigma> p) (\<sigma> a)"
      using field_hom_on.poly_map_poly_hom[OF homs pE aE] by (simp add: xp)
    also have "\<dots> = poly (map_poly \<tau> p) (\<tau> a)"
      using eq fixFs fixFt homs homt p sfF by (metis map_poly_fix)
    also have "\<dots> = \<tau> x"
      using field_hom_on.poly_map_poly_hom[OF homt pE aE] by (simp add: xp)
    finally show ?thesis .
  qed
  \<comment> \<open>Both maps are extensional on @{term E}, so agreeing on @{term E} makes them equal.\<close>
  then show ?thesis
    using s t unfolding E_def field_auto_mem_iff by (meson PiE_ext)
qed
...
proof -
  ...
  define E where "E \<equiv> eval_img F a"
  define R where "R \<equiv> {r. poly m r = 0}"
  have sfE: "Subfield E" unfolding E_def by (rule Subfield.subfield_eval_img[OF sfF alg])
  ...
  proof (rule antisym)
    ...
    then show "E \<subseteq> generate_field (F \<union> R)"
      unfolding E_def
      using Subfield.eval_img_eq_generate_field[OF sfF alg] by (metis generate_field_mono)
  ...
  proof
    ...
    proof (rule antisym)
      ...
      show "(\<lambda>\<sigma>. \<sigma> a) ` field_auto E F \<subseteq> R"
        using FE R_def aE field_auto_maps_root mF mroot sfE sfF by blast
      ...
      show "R \<subseteq> (\<lambda>\<sigma>. \<sigma> a) ` field_auto E F"
        using galois_action_transitive[OF sfF finR mF irr _ closed aR] E_gen aR 
        by (force simp add: R_def)
    qed
  qed
  then show ?thesis
    using E_def R_def bij_betw_same_card m_def by blast
qed
...
lemma (in Subfield) poly_over_pderiv:
  ...
  shows "pderiv p \<in> poly_over K"
    using poly_overI assms mult_closed of_nat_closed poly_over_coeff by (metis coeff_pderiv)
...
proof -
  ...
  have pmF: "pderiv m \<in> poly_over F"
    using Subfield.poly_over_pderiv is_minpoly_def ism sfF by blast
  ...
  have "\<not> (poly m r = 0 \<and> poly (pderiv m) r = 0)" for r
    using bez pmnz by (metis mult_cancel_left1 poly_squarefree_decomp rsquarefree_roots)
  then show ?thesis
    using m_def rsquarefree_roots by blast
qed
...
proof -
  define m where "m \<equiv> minpoly F a"
  have mnz: "m \<noteq> 0"
    by (simp add: Subfield.minpoly_nonzero alg m_def sfF)
  have monic: "lead_coeff m = 1"
    by (simp add: Subfield.minpoly_monic alg m_def sfF)
  have "rsquarefree m"
    by (simp add: alg m_def rsquarefree_minpoly sfF)
  \<comment> \<open>Squarefree decomposition into distinct monic linear factors.\<close>
  then have decomp: "smult (lead_coeff m) (\<Prod>z | poly m z = 0. [:- z, 1:]) = m"
   by (intro complex_poly_decompose_rsquarefree)
  have "degree m = degree (\<Prod>z | poly m z = 0. [:- z, 1:])"
    using decomp monic by (metis smult_1_left)
  also have "\<dots> = card {z. poly m z = 0}" 
    using mnz by (simp add: poly_roots_finite degree_prod_eq_sum_degree)
...
proof -
  ...
  have finR: "finite R"
    using R_def irr irreducible_over_def poly_roots_finite by blast
  have E_gen: "E = generate_field (F \<union> R)"
  proof (rule antisym)
    show "E \<subseteq> generate_field (F \<union> R)"
      using Subfield.eval_img_eq_generate_field[OF sfF alg] mroot
      by (simp add: E_def R_def generate_field_mono)
    show "generate_field (F \<union> R) \<subseteq> E"
      using FE RE by (intro generate_field_least[OF sfE]) auto
  qed
  have action_bij: "bij_betw (\<lambda>\<sigma>. \<sigma> a) (field_auto E F) R"
    unfolding bij_betw_def
  proof
    show "inj_on (\<lambda>\<sigma>. \<sigma> a) (field_auto E F)"
      unfolding inj_on_def
      using field_auto_determined_by_gen[OF sfF alg] by (simp add: E_def)
    show "(\<lambda>\<sigma>. \<sigma> a) ` field_auto E F = R"
    proof
      show "(\<lambda>\<sigma>. \<sigma> a) ` field_auto E F \<subseteq> R"
        using FE R_def aE field_auto_maps_root mF mroot sfE sfF by blast
      show "R \<subseteq> (\<lambda>\<sigma>. \<sigma> a) ` field_auto E F"
        using E_gen mroot galois_action_transitive[OF sfF finR mF irr _ alg_closed_imp_poly_has_root] 
        by (force simp: R_def)
    qed
  qed
  then show ?thesis
...
proof -
  have card_group_roots:
      "card (field_auto (eval_img F a) F) = card {r. poly (minpoly F a) r = 0}"
    by (rule galois_simple_normal_card_alg_closed[OF sfF alg normal])
  have sep_minpoly: "rsquarefree (minpoly F a)"
    using Subfield.eval_img_self alg sep separable_extension_def sfF by blast
  have minpoly0: "minpoly F a \<noteq> 0"
    by (rule Subfield.minpoly_nonzero[OF sfF alg])
  then have card_roots_degree:
      "card {r. poly (minpoly F a) r = 0} = ext_degree F a"
    by (metis sep_minpoly card_roots_eq_degree_alg_closed ext_degree_def)
...
proof -
  ...
  proof (rule subset_antisym)
    ...
    show "generate_field (F \<union> R) \<subseteq> K"
      by (simp add: T.base_subset T.ext.Subfield_axioms generate_field_least rootsK)
  qed
  show ?thesis
    using finite_galois_group[OF T.ext.Subfield_axioms T.base.Subfield_axioms T.base_subset pF]
    using Kgen R_def finR by blast
qed
...
proof -
  ...
  have "poly (minpoly F a) a = 0"
    by (rule T.base.minpoly_root[OF alg])
  then have roots_simple: "{r. poly (minpoly F a) r = 0} \<subseteq> eval_img F a"
    using Kdef T.base.minpoly_over aK alg irr normal normal_extensionD poly_root_set_def
    by blast
  have card_simple: "card (field_auto (eval_img F a) F) = ext_degree F a"
    using Kdef T.base.Subfield_axioms alg galois_simple_normal_separable_degree roots_simple sep
    by blast
  show ?thesis
    using Kdef T.primitive_element_degree card_simple prim by presburger
qed
